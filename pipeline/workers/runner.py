...
VERSION = "5.9.7"  # v5.9.7: back-planned stage deadlines, opt-in auto-approve, cost-per-published-post metric  # v5.9.6: free-ladder FLOOR (merge not replace), SLA-aware paid escalation, in-flight age-out, replan dedupe, daemon heartbeat, 14:00 Asia/Dubai SLA, full-fidelity diagnostics  # v5.9.5: demand-governed ideation (windowed idempotency), SLA plan/monitor/self-heal, $0 paused-account prep, fair claim ordering, empty-topic root-cause fix, human_desk 20s->120s  # v5.9.4: $25/month per-account hard cap in ceo_decide  # v5.8.5: SHIP-BEST gate (grader ships best attempt >=7.0 instead of final-reject), provider inventory on boot, prepare-docs-while-paused.  # v5.8.3: scouted skills for 8 depts + free routing for distribution/research/community. v5.8.2: council, approval->render bridge, lessons loop, ceo fix  # v5.7: soft-pause (pause intake, finish in-flight), docs library+editor in web, /api/version
