-- ============================================================
-- Agent-X v2 — ONE-SHOT FULL SETUP
-- Run this ONCE in Supabase SQL Editor. Safe to re-run.
-- Includes: marketplace/affiliate, trend scout, multi-project,
-- account docs, planned posts, MCP connections.
-- ============================================================

-- ============ 1) TREND SCOUT ============
create table if not exists trend_items (
  id bigint generated always as identity primary key,
  tenant_id text not null default 'me', niche text not null default 'ai_tools',
  platform text not null default 'news',
  title text not null, url text not null, author text,
  views bigint not null default 0, engagement bigint not null default 0,
  heat int not null default 0, published_at timestamptz, scraped_at timestamptz not null default now()
);
...
-- ============ 2) PROJECTS (multi-niche per user) ============
create table if not exists public.projects (
  ...
  cta text, created_at timestamptz not null default now()
);
...
-- ============ 3) MARKETPLACE + AFFILIATE ============
create table if not exists public.marketplace_agents (
  id uuid primary key default gen_random_uuid(),
  slug text unique not null, name text not null, tagline text, description text, category text,
...
create table if not exists public.affiliate_clicks (
  id bigint generated always as identity primary key,
  code text not null, agent_slug text, created_at timestamptz not null default now()
);
...
create table if not exists public.agent_leads (
  id uuid primary key default gen_random_uuid(),
  agent_slug text not null, name text not null, email text not null, company text, message text, ref_code text,
...
-- seed 6 demo marketplace agents (only if table is empty)
...
select * from (values
  ('aria-support','Aria — Support Desk Agent','Answers customer questions from your own docs, 24/7.',
   ...
   '[{"q":"What are your shipping times?","a":"Standard shipping is 3–5 business days inside the EU — pulled from your store policy. Want the express options too?"}]'::jsonb),
  ('scout-sdr','Scout — Outbound SDR Agent','Researches leads and drafts personalized outreach for your review.',
   ...
   '[{"q":"Find SaaS in Berlin hiring support","a":"I would search public job boards and company sites, then draft a one-line opener referencing each posting. Drafts go to your review queue — you send, not me."}]'::jsonb),
  ('quill-content','Quill — Content Writer Agent','Blog posts, product pages and newsletters in your voice.',
   ...
   '[{"q":"Write an intro about our new pricing","a":"Give me the pricing page and one older post you like, and I will match its rhythm. Every number links back — no invented stats."}]'::jsonb),
  ...
  ('clara-booking','Clara — Booking & Scheduling Agent','Turns inquiries into booked calls without back-and-forth.',
   'Clara connects to your calendar, offers real open slots, handles reschedules, and sends reminders — only inside availability windows you define.',
   'Operations',190,
   '["Calendar integration","Real availability only","Reschedule handling","Reminder messages","Timezone aware"]'::jsonb,
   '[{"q":"Book me with a client next week","a":"I see Tue 10:00, Wed 14:30, Thu 09:00 open. When the client picks, it lands on your calendar with a reminder."}]'::jsonb),
  ('ledger-ops','Ledger — Back-office Data Agent','Cleans, tags and files the data work nobody wants.',
   'Ledger takes recurring data chores — invoice tagging, CRM cleanup, spreadsheet merging — and runs them on schedule with a change log you can audit.',
   'Operations',240,
   '["Scheduled data chores","Dry-run preview","Full change log","Spreadsheet & CSV native","Duplicate detection"]'::jsonb,
   '[{"q":"Clean my messy contact sheet","a":"Upload it and I will show a dry-run first. Nothing changes until you approve the preview."}]'::jsonb)
) as v(s,n,t,d,c,p,cap,ds)
where not exists (select 1 from public.marketplace_agents where slug = v.s);

-- ============ 4) ACCOUNTS (brand accounts under projects) ============
...
-- ============ 5) ACCOUNT DOCUMENTS (business plan / brand / tone / visuals / rules) ============
create table if not exists public.account_documents (
  ...
  content text not null, agent text not null default 'architect', version int not null default 1,
...
-- ============ 6) ACCOUNT POSTS (planned reels, hooks, scripts, hashtags) ============
create table if not exists public.account_posts (
  id uuid primary key default gen_random_uuid(),
  account_id uuid not null references public.project_accounts(id) on delete cascade,
  post_type text not null check (post_type in ('reel','carousel','image','short')),
  title text not null, hook text, script text, caption text not null, visual_prompt text,
  hashtags text[] not null default '{}', duration_seconds int default 30,
...
create trigger trg_touch_accounts before update on public.project_accounts for each row execute function public.touch_updated_at();
drop trigger if exists trg_touch_docs on public.account_documents;
create trigger trg_touch_docs before update on public.account_documents for each row execute function public.touch_updated_at();
drop trigger if exists trg_touch_posts on public.account_posts;
create trigger trg_touch_posts before update on public.account_posts for each row execute function public.touch_updated_at();

-- ============ 8) MCP CONNECTIONS (Claude / Cursor / ChatGPT) ============
create table if not exists public.mcp_connections (
  id uuid primary key default gen_random_uuid(),
  user_id uuid references auth.users(id) on delete cascade not null,
  provider text not null, label text, access_token text not null,
...
-- ============ FINAL VERIFY ============
select 'v2 setup complete' as status,
  (select count(*) from trend_items) as trend_items,
  (select count(*) from projects) as projects,
  (select count(*) from project_accounts) as accounts,
  (select count(*) from account_documents) as documents,
  (select count(*) from account_posts) as posts,
