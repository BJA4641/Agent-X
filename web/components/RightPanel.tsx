...
export default async function RightPanel() {
  ...
  const todos = [
    ...
    { label:"Open your first track", href:"/dashboard/projects", done:false },
  ];
  const openTodos = todos.filter(t => !t.done);

  const agentColor: Record<string,string> = {
    brain:"var(--draft)", visuals:"var(--scheduled)", voice:"var(--approved)",
    qa:"var(--failed)", strategy:"#a78bfa", planner:"#60a5fa", publisher:"#34d399",
    system:"var(--dim)", architect:"#a78bfa", strategist:"#eab308",
    grader:"#ef4444", scout:"#f97316", composer:"#22d3ee",
    analyst:"#8b5cf6", community:"#14b8a6", digest:"#64748b",
    budget:"#eab308",
  };

  // Determine current path to avoid "open feed" on the workspace page itself
  const headersList = headers();
  const path = headersList.get("x-next-pathname") || headersList.get("referer") || "";
  const onWorkspace = path.includes("/dashboard/workspace");
    ...
    <aside style={{borderLeft:"1px solid var(--line)",padding:"20px 0 20px 20px",display:"flex",flexDirection:"column",gap:24,fontSize:13}}>
      <div className="card" style={{margin:0,background:"linear-gradient(135deg, rgba(16,185,129,0.08), rgba(59,130,246,0.08))"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
          <b>Wallet</b><span className="tag">live</span>
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {todos.map(t => (
            <Link key={t.label} href={t.href} style={{
              ...
              <span style={{width:14,height:14,borderRadius:4,border:"1.5px solid "+(t.done?"var(--approved)":"var(--line)"),
      ...
      <div>
        ...
        {pending.length===0 ? (
          <p className="note" style={{margin:0,fontSize:12}}>Nothing in your queue yet. Run the Studio or pick an active account to generate drafts.</p>
        ) : (
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
      ...
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:8}}>
          ...
          {!onWorkspace && <Link href="/dashboard/workspace" className="tag" style={{textDecoration:"none",fontSize:11}}>open feed →</Link>}
        </div>
        {events.length===0 ? (
          <div style={{padding:10,border:"1px dashed var(--line)",borderRadius:8}}>
            <p className="note" style={{margin:0,fontSize:12}}>
              Agents are idle. Resume an account from <Link href="/dashboard/projects" style={{color:"var(--scheduled)"}}>Projects</Link> to wake them up.
            </p>
          </div>
        ) : (
